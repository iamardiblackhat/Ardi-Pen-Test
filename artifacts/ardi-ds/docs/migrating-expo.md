# Migrating Expo UI to Ardi Design System

Read `artifacts/ardi-ds/docs/AGENTS.md` and
`artifacts/ardi-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/ardi-ds/lib/native-theme` and
  `@workspace/ardi-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/ardi-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/ardi-ds` and retains only
