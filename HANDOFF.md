...
- **Threat intelligence / OpenCTI** — deployed on a dedicated GCP VM (`ardi-threat-intel`, `europe-west1-c`) using the production container stack in `deploy/opencti/`. The customer-facing ARDI workspace exposes live searchable campaigns, actors, malware, indicators, vulnerabilities, reports, and attack techniques at `/intelligence`; ARDI chat uses the same live GraphQL feed. The underlying operations console is available at `https://104.199.94.244.sslip.io`. Credentials remain server-side and must never be committed or exposed to customers.
