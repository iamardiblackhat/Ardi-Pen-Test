 ...
 *
 * SECURITY: none of these accept an org/tenant identifier as a model-supplied
 * parameter. Once multi-tenancy lands (Phase 1), the org is bound from the
 * authenticated session at construction time. If it were a tool argument, a
 * prompt injection hidden in a scanned host's HTTP banner — attacker-controlled
 * text that ARDI reads — could ask him to fetch another customer's findings.
 */

const SEVERITIES = ["critical", "high", "medium", "low", "info"] as const;

export const listFindings = betaZodTool({
  name: "list_findings",
  description:
  ...
  run: async ({ severity, status, limit }) => {
    let rows = await db.select().from(findingsTable).orderBy(desc(findingsTable.createdAt));
...
export const getFinding = betaZodTool({
  name: "get_finding",
  description:
...
export const listAssets = betaZodTool({
  name: "list_assets",
  description:
...
export const listScans = betaZodTool({
  name: "list_scans",
  description:
  ...
  run: async ({ limit }) => {
    const rows = await db.select().from(scansTable).orderBy(desc(scansTable.createdAt));
    const assets = await db.select().from(assetsTable);
    const names = new Map(assets.map((a) => [a.id, a.name]));
...
export const getSecuritySummary = betaZodTool({
